...
function [xe,te,x,t,energies,indices,vinterms] = onestep4(w, x0, bumps, ctrls, latePushOffFlag, varargin);

%this is same as onestep3 but it is used for to find the linearized time so
%only outputs end time
%
...
for i = 1:size(ctrls, 2)  
  ...
  [xe_temp, energyinfo, vinterm] = s2stransitionTemp(x0, w, parms, bump, ctrl(1));  %ctrl(1)%<---------------------------------
