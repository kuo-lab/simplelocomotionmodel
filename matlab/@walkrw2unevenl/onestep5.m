...
function [xe,te,x,t,energies,indices,vinterms] = onestep5(w, xState, bumps, ctrls, latePushOffFlag, varargin)

%this is same as onestep4 but it is used to find the augmented state that
%involves time and timedot on top theta and thetadot, now it is for just
%one step
...
% setTfFlag = 0;
% if t0 ~= size(ctrls, 2)*nominalTime
%     setTfFlag = 1;
% end
% 
% setTdotfFlag = 0;
...
for i = 1:size(ctrls, 2)  
  ...
  [xe_temp, energyinfo, vinterm] = s2stransitionTemp(x0, w, parms, 0, ctrl(1));  
